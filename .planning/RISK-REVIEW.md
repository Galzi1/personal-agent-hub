...
- **If wrong:** Phases 2 and 3 (canonical story formation and ranked digest composition) produce low-quality output. The user gets a digest that is technically correct but reads poorly or makes bad ranking decisions. Remediation is cheap - swap models in `config/models.yaml`.
- **Testable?** Yes (secret). Plan 00-02 runs representative ranking and summarization tasks against the starter matrix before Phase 2 implementation begins. If a starter fails, the config-driven layer (D-17) absorbs the swap without code changes.
...
|----|----------|-------------|---------|------|----------|----------|-----------|------------|-------------|------------|
| **R1** | Technical | ~~**MicroClaw proves insufficient or breaks**~~ **CLOSED 2026-04-23** via Plan 00-01 GO - all 4 smoke tests PASS on Windows 11 x86_64. Residual: @-mention routing contract (F1) constrains Phase 2+ inbound flows; logged as D-23 in Phase 0 CONTEXT.md. | N/A - closed | Medium | Critical | **CLOSED** | See `.planning/phases/00-validation-spikes/00-01-SPIKE-RESULTS.md` | Validation complete | NanoClaw fallback path no longer active | A1 |
...
**Known Unknowns:** ~~R1 (MicroClaw sufficiency - CLOSED)~~, R3 (OpenRouter-hosted model quality), R5 (dedup quality), R8 (Discord limitations) - we know these are risks but cannot quantify them without testing.
...
1. ~~**R1 - MicroClaw as a single point of failure (Critical).**~~ **CLOSED 2026-04-23** via Plan 00-01 GO. Residual: @-mention routing contract (F1/D-23) constrains Phase 2+ inbound flows. Phase 1 scope is outbound-only (D-24), so F1 does not block Phase 1.

2. **R2 - Long time to first useful digest (High).** Three full phases before the user gets a working daily brief is too long for a personal project. Phase 1.5 "thin digest" milestone mitigates. Still the top active risk.
...
2. **Add a "thin digest" milestone** to Phase 1 or as Phase 1.5. After ingestion works, post a simple formatted list of ingested items to Discord daily. This gives the user a tangible daily artifact to react to, even before dedup and ranking exist. It also validates the end-to-end pipeline (schedule → ingest → format → deliver) early. *(Phase 1.5 added to roadmap.)*

3. **Evaluate OpenRouter-hosted model quality** on 10-15 representative tasks (ranking relevance, summarizing an AI news item, explaining "why it matters") before Phase 2 planning begins. Plan 00-02 scoped and pending execution; $20 spend cap per D-22. If a starter fails, swap the failing task's model in `config/models.yaml` (no code changes).
...
- ~~**What Ollama model does the user plan to run?**~~ **RESOLVED 2026-04-24 via Phase 0 D-13/D-18:** Local Ollama chat path dropped; per-task OpenRouter models selected (Gemini 3 Flash Preview / Nemotron 3 Super / GPT-5.4 / Cohere embed-v3.0). Plan 00-02 verifies each on OpenRouter `/v1/models`.
- ~~**What is the user's Rust proficiency?**~~ **RESOLVED** - R1 closed, "fork and own MicroClaw" mitigation no longer on the critical path.
...
*Revised: 2026-04-24 - R1 CLOSED (Plan 00-01 GO); R3 reworded for OpenRouter; A2 rewritten for OpenRouter; Scenario 4 inverted.*
*Inputs: ROADMAP.md, PROJECT.md, REQUIREMENTS.md, STATE.md, ARCHITECTURE.md, STACK.md, FEATURES.md, PITFALLS.md, SUMMARY.md, 01-CONTEXT.md, 01-RESEARCH.md, 01-VALIDATION.md, 01-DISCUSSION-LOG.md, architecture-repository-mapping.md, visible-routing-architecture.md, layered-memory-model.md, 00-CONTEXT.md, 00-01-SPIKE-RESULTS.md, 00-01-SUMMARY.md*
